...
const PHOTO_MAX_DIM = 1000;  // longest side, px — big enough that the PDF/print output stays sharp
const PHOTO_QUALITY = 0.88;  // JPEG quality
...
let state = {
  ...
  activeItemId: null,
  selectedIds: new Set(), // survives re-renders, unlike raw DOM checkbox state
};
...
document.getElementById("selectAllBtn").addEventListener("click", () => {
  ...
  const allSelected = list.every((it) => state.selectedIds.has(it.id));
  list.forEach((it) => {
    if (allSelected) state.selectedIds.delete(it.id);
    else state.selectedIds.add(it.id);
  });
  renderItems();
});
...
document.getElementById("exportSelected2upBtn").addEventListener("click", exportSelectedTwoUpPdf);
...
function updateSelectedCount() {
  ...
  if (el) el.textContent = state.selectedIds.size ? `${state.selectedIds.size} selected` : "";
}
...
function renderItems() {
  ...
  // Drop selections for items that no longer exist (e.g. after Clear all data)
  const liveIds = new Set(state.items.map((it) => it.id));
  [...state.selectedIds].forEach((id) => { if (!liveIds.has(id)) state.selectedIds.delete(id); });
...
function rowHtml(it) {
  ...
  return `
    <tr data-id="${it.id}">
      <td><input type="checkbox" class="rowSelect" ${state.selectedIds.has(it.id) ? "checked" : ""} onchange="toggleSelected('${it.id}', this.checked)"></td>
...
function renderBarcodeToCanvas(canvas, rawValue) {
  ...
  // Rendered at high module width/height, then scaled DOWN by CSS to fit the
  // card. Downscaling stays sharp; upscaling is what made the old output blurry.
  const opts = { displayValue: false, margin: 6, height: 130, width: 5 };
...
function barcodeBoxHtml(key, label, value, forExport = false) {
  const P = PRINT_COLORS;
  const labelStyle = forExport ? ` style="color:${P.inkSoft};"` : "";
  if (!String(value || "").trim()) {
    return `<div class="barcode-box empty"${forExport ? ` style="color:${P.inkSoft};"` : ""}>${label}<br>— not provided —</div>`;
  }
  return `
    <div class="barcode-box"${key !== "case" && forExport ? ` style="border-left-color:${P.ink};"` : ""}>
      <div class="label"${labelStyle}>${label}</div>
      <canvas id="bc-${key}"></canvas>
      <div class="digits">${escapeHtml(value)}</div>
      <div class="fmt"${labelStyle} id="fmt-${key}"></div>
    </div>
  `;
}

// Literal, guaranteed-plain color values for the exported card — no CSS
// variables, no var()/oklch() indirection anywhere html2canvas has to parse.
// The on-screen editor view keeps using the themed CSS classes as normal;
// only the export path (forExport=true) uses these hardcoded values.
const PRINT_COLORS = {
  ink: "#14161A", inkSoft: "#5C6068", line: "#E3E3DE",
  petrol: "#C2410C", amber: "#5C6068", paper: "#FAFAF7", white: "#FFFFFF",
};

function labelCardHtml(it, forExport = false) {
  const dims = (it.width || it.length || it.height)
    ? `${it.width || "—"} × ${it.length || "—"} × ${it.height || "—"}${it.dimUnit ? " " + it.dimUnit : ""}`
    : "—";
  const P = PRINT_COLORS;

  // Photo area: exported version is a static image (or nothing) — the
  // upload button and link field are edit-mode-only and never belong in a
  // customer-facing PDF.
  const photoBlock = it.imageUrl
    ? `<img src="${escapeHtml(toDirectImageUrl(it.imageUrl))}" crossorigin="anonymous" onerror="this.replaceWith(Object.assign(document.createElement('div'),{className:'no-image',textContent:'Image failed to load'}))">`
    : forExport
      ? `<div class="no-image"${forExport ? ` style="border:1px dashed ${P.line};color:${P.inkSoft};"` : ""}>No image provided</div>`
      : `<div class="no-image">No image link provided</div>`;
  const uploadControls = forExport ? "" : `
  ...
  return `
    <div class="label-card" id="labelCardPrintable" ${forExport ? `style="border-color:${P.ink};"` : ""}>
      <div class="label-head" ${forExport ? `style="background:${P.ink};border-bottom-color:${P.ink};"` : ""}>
        <div class="company" ${forExport ? `style="color:${P.white};"` : ""}>${escapeHtml(COMPANY_NAME)}</div>
        <div class="doc-id" ${forExport ? `style="color:${P.white};"` : ""}>PRODUCT LISTING · SAP ${escapeHtml(it.sapCode || "—")}</div>
      </div>
      <div class="label-body">
        <div class="label-media" ${forExport ? `style="border-right-color:${P.line};"` : ""}>
          ${photoBlock}${uploadControls}
        </div>
        <div class="label-fields">
          <div class="field-row" ${forExport ? `style="border-bottom-color:${P.line};"` : ""}><div class="k" ${forExport ? `style="color:${P.inkSoft};"` : ""}>FPC / Principal Code</div><div class="v">${escapeHtml(it.principalCode) || "—"}</div></div>
          <div class="field-row" ${forExport ? `style="border-bottom-color:${P.line};"` : ""}><div class="k" ${forExport ? `style="color:${P.inkSoft};"` : ""}>Brand</div><div class="v">${escapeHtml(it.brand) || "—"}</div></div>
          <div class="field-row desc"><div class="k" ${forExport ? `style="color:${P.inkSoft};"` : ""}>Description</div></div>
          <div class="field-row desc" style="border-bottom:1px dotted ${forExport ? P.line : "var(--line)"};"><div class="v" style="font-weight:500;">${escapeHtml(it.sapDescription) || "—"}</div></div>
          <div class="field-row price-row" ${forExport ? `style="border-bottom-color:${P.line};"` : ""}><div class="k" ${forExport ? `style="color:${P.inkSoft};"` : ""}>Case Price (BHD)</div><div class="v" ${forExport ? `style="color:${P.petrol};"` : ""}>${escapeHtml(it.casePrice) || "—"}</div></div>
          <div class="field-row price-row" ${forExport ? `style="border-bottom-color:${P.line};"` : ""}><div class="k" ${forExport ? `style="color:${P.inkSoft};"` : ""}>Piece Price (BHD)</div><div class="v" ${forExport ? `style="color:${P.petrol};"` : ""}>${escapeHtml(it.piecePrice) || "—"}</div></div>
          <div class="field-row" ${forExport ? `style="border-bottom-color:${P.line};"` : ""}><div class="k" ${forExport ? `style="color:${P.inkSoft};"` : ""}>Piece Dimensions (W×L×H)</div><div class="v">${dims}</div></div>
        </div>
      </div>
      <div class="customs-strip" ${forExport ? `style="border-top-color:${P.ink};"` : ""}>
        <div class="cell">
          <div class="k" ${forExport ? `style="color:${P.amber};"` : ""}>Source / Country of Origin</div>
          <div class="v">${escapeHtml(it.countryOfOrigin) || "— not provided —"}</div>
        </div>
        <div class="cell" ${forExport ? `style="border-left-color:${P.ink};"` : ""}>
          <div class="k" ${forExport ? `style="color:${P.amber};"` : ""}>HS Code</div>
          <div class="v">${escapeHtml(it.hsCode) || "— not provided —"}</div>
        </div>
      </div>
      <div class="barcode-strip" ${forExport ? `style="border-top-color:${P.ink};"` : ""}>
        ${barcodeBoxHtml("case", "Case Barcode", it.caseBarcode, forExport)}
        ${barcodeBoxHtml("outer", "Bundle Barcode", it.outerBarcode, forExport)}
        ${barcodeBoxHtml("piece", "Piece Barcode", it.pieceBarcode, forExport)}
      </div>
    </div>
  `;
}
...
// ---------------------------------------------------------------------
// Generic timeout wrapper — used so one slow/hanging item (typically a
// product photo URL that never resolves) can't stall an entire batch export.
function withTimeout(promise, ms, label) {
  return Promise.race([
    promise,
    new Promise((_, reject) => setTimeout(() => reject(new Error(`${label || "operation"} timed out after ${ms / 1000}s`)), ms)),
  ]);
}

// html2canvas (even the "pro" fork) can throw on certain modern CSS color
// serializations depending on the browser/GPU color-management path. This
// sidesteps the whole problem: right before rendering, every element's
// resolved color gets normalized through a canvas 2d context (which always
// outputs a plain rgb()/rgba() string, whatever format it was given) and
// written back as a literal inline style — so html2canvas never has to
// parse anything exotic, regardless of what Chrome version is doing.
...
function flattenColorsForExport(root) {
  ...
  [root, ...root.querySelectorAll("*")].forEach((el) => {
    ...
    props.forEach((p) => {
      ...
      if (val && val !== "rgba(0, 0, 0, 0)" && val !== "transparent") {
        try { el.style[p] = normalizeColor(val); } catch (e) { /* leave as-is */ }
      }
    });
  });
}

// Renders one item's listing card off-screen and returns the resulting
// canvas. Shared by the single-item PDF export and the 2-up bulk PDF export
// so both produce pixel-identical cards from one code path.
async function renderItemCanvas(it) {
  ...
  host.innerHTML = labelCardHtml(it, true);
  ...
  // Wait for the product photo to actually finish loading (success or
  // failure) instead of a blind fixed delay — bounded so a slow/unreachable
  // image URL can't stall this indefinitely. If there's no image, or it's
  // already resolved, this returns immediately.
  const imgEl = host.querySelector(".label-media img");
  if (imgEl && !imgEl.complete) {
    await new Promise((resolve) => {
      const done = () => resolve();
      imgEl.addEventListener("load", done, { once: true });
      imgEl.addEventListener("error", done, { once: true });
      setTimeout(done, 4000); // don't wait forever on a slow/broken link
    });
  }

  try {
    const cardEl = host.querySelector("#labelCardPrintable");
    flattenColorsForExport(cardEl);
    return await withTimeout(
      html2canvas(cardEl, { scale: 3, backgroundColor: "#ffffff", useCORS: true }),
      15000,
      "Rendering listing"
    );
  } finally {
    // Always clean up the off-screen element, even if rendering timed out —
    // otherwise a hung item leaves a stray node behind on every retry.
    host.remove();
  }
}

async function itemToPdfBlob(it) {
  const canvas = await renderItemCanvas(it);
...
// ---------------------------------------------------------------------
// Excel cells hard-cap at 32,767 characters — a base64 data URL from a
// device-uploaded photo can easily be 5-10x that, which crashes the whole
// workbook write. Base64 image data isn't useful in a spreadsheet cell
// anyway (not clickable/viewable), so it's replaced with a clear note
// instead of being truncated into garbage.
const EXCEL_CELL_LIMIT = 32000; // a little under the real 32767 limit, for safety margin
function excelSafeImageValue(imageUrl) {
  if (!imageUrl) return "";
  if (imageUrl.startsWith("data:")) {
    return "[Photo attached in-app via device upload — open the item in the app or use the PDF export to view it; not included here since it's too large for a spreadsheet cell]";
  }
  return imageUrl.length > EXCEL_CELL_LIMIT ? imageUrl.slice(0, EXCEL_CELL_LIMIT) + "…[truncated]" : imageUrl;
}
// Defensive catch-all: whatever the field, never let a value overflow the
// Excel cell limit — better to visibly truncate than crash the export.
function excelSafeValue(value) {
  const s = String(value ?? "");
  return s.length > EXCEL_CELL_LIMIT ? s.slice(0, EXCEL_CELL_LIMIT) + "…[truncated]" : s;
}

function itemToWorkbook(it) {
  const rows = [
    [COMPANY_NAME],
    ["FPC / Principal Code", excelSafeValue(it.principalCode)],
    ["Brand", excelSafeValue(it.brand)],
    ["SAP Code", excelSafeValue(it.sapCode)],
    ["SAP Description", excelSafeValue(it.sapDescription)],
    ["Case Barcode", excelSafeValue(it.caseBarcode)],
    ["Outer / Bundle Barcode", excelSafeValue(it.outerBarcode)],
    ["Piece Barcode", excelSafeValue(it.pieceBarcode)],
    ["Case Selling Price – BHD", excelSafeValue(it.casePrice)],
    ["Piece Selling Price – BHD", excelSafeValue(it.piecePrice)],
    ["Piece Dimension (W)", excelSafeValue(it.width)],
    ["Piece Dimension (L)", excelSafeValue(it.length)],
    ["Piece Dimension (H)", excelSafeValue(it.height)],
    ["Dimension Unit", excelSafeValue(it.dimUnit)],
    ["Source / Country of Origin", excelSafeValue(it.countryOfOrigin)],
    ["HS Code", excelSafeValue(it.hsCode)],
    ["Image Link", excelSafeImageValue(it.imageUrl)],
  ];
...
async function exportSelectedZip() {
  const ids = [...state.selectedIds].filter((id) => state.items.some((it) => it.id === id));

  if (!ids.length) {
    toast("Select at least one item first — tick the checkbox on the left of each row.");
    return;
  }
  ...
  try {
    ...
    for (const id of ids) {
      ...
      btn.textContent = `Generating ${done}/${ids.length}…`;
      toast(`Generating ${done} of ${ids.length}: ${name}…`);
      ...
      try {
        ...
        console.error("Excel generation failed for", name, e);
        failures.push(`${name} (Excel)`);
      }
    }
    ...
    if (failures.length) {
      toast(`Downloaded with ${failures.length} issue${failures.length === 1 ? "" : "s"} — check console for details.`);
    } else {
      toast(`Download ready — ${ids.length} PDF${ids.length === 1 ? "" : "s"} + ${ids.length} Excel file${ids.length === 1 ? "" : "s"}, named by SAP code.`);
    }
  } catch (e) {
    console.error("Export failed:", e);
    toast(`Export failed: ${e.message || "unknown error"} — check the browser console (F12) for details.`);
  } finally {
    btn.disabled = false;
...
// ---------------------------------------------------------------------
// Bulk export: selected items -> ONE PDF, two listing cards per A4 page
// ---------------------------------------------------------------------
// Unlike exportSelectedZip (one PDF file per item), this produces a single
// multi-page PDF with two cards stacked per page and a dashed cut/fold
// guide between them — meant for quick printing rather than per-SAP-code
// filing.
async function exportSelectedTwoUpPdf() {
  const ids = [...state.selectedIds].filter((id) => state.items.some((it) => it.id === id));
  ...
  const btn = document.getElementById("exportSelected2upBtn");
  const originalLabel = btn.textContent;
  btn.disabled = true;
  toast(`Generating ${ids.length} listing${ids.length === 1 ? "" : "s"} (2-up)…`);
  ...
  try {
    ...
    const pageHeight = pdf.internal.pageSize.getHeight();
    const margin = 24;
    const gap = 16; // space between the two cards, where the cut/fold guide sits
    const slotW = pageWidth - margin * 2;
    const slotH = (pageHeight - margin * 2 - gap) / 2;
    ...
    for (let i = 0; i < ids.length; i += 2) {
      ...
      const pairIds = [ids[i], ids[i + 1]].filter((id) => id !== undefined);
      for (let slot = 0; slot < pairIds.length; slot++) {
        const it = state.items.find((x) => x.id === pairIds[slot]);
        if (!it) continue;
        done++;
        btn.textContent = `Generating ${done}/${ids.length}…`;
        toast(`Generating ${done} of ${ids.length}: ${outputFilename(it)}…`);
        ...
        try {
          ...
          const slotTop = margin + slot * (slotH + gap);
          const x = margin + (slotW - w) / 2;
          const y = slotTop + (slotH - h) / 2;
          pdf.addImage(imgData, "PNG", x, y, w, h);
        } catch (e) {
          console.error("2-up PDF: rendering failed for", it && it.sapCode, e);
          failures.push(outputFilename(it));
        }
      }

      // Dashed cut/fold guide between the two cards, only when a page
      // actually has two of them (last page may have just one).
      if (pairIds.length === 2) {
        const midY = margin + slotH + gap / 2;
        pdf.setDrawColor(180);
        pdf.setLineDash([4, 4], 0);
        pdf.line(margin, midY, pageWidth - margin, midY);
        pdf.setLineDash([]);
      }
    }

    pdf.save(`scannable-listings-2up-${new Date().toISOString().slice(0, 10)}.pdf`);

    if (failures.length) {
      toast(`Downloaded with ${failures.length} issue${failures.length === 1 ? "" : "s"} — check console for details.`);
    } else {
      toast(`Download ready — ${ids.length} listing${ids.length === 1 ? "" : "s"}, 2 per page.`);
    }
  } catch (e) {
    console.error("2-up export failed:", e);
    toast(`Export failed: ${e.message || "unknown error"} — check the browser console (F12) for details.`);
  } finally {
    btn.disabled = false;
    btn.textContent = originalLabel;
