...
const PRINT_COLORS = {
  ink: "#1A1A18", inkSoft: "#4A4A44", line: "#D8D5C9",
  petrol: "#B6431A", amber: "#D68A00", paper: "#FAFAF7", white: "#FFFFFF",
};
...
function labelCardHtml(it, forExport = false) {
  ...
  return `
    <div class="label-card" id="labelCardPrintable" ${forExport ? `style="border-color:${P.ink};"` : ""}>
      <div class="label-head" ${forExport ? `style="background:${P.petrol};border-bottom-color:${P.ink};"` : ""}>
        <div class="company" ${forExport ? `style="color:${P.white};"` : ""}>${escapeHtml(COMPANY_NAME)}</div>
        <div class="doc-id" ${forExport ? `style="color:${P.white};"` : ""}>SAP ${escapeHtml(it.sapCode || "—")}</div>
      </div>
...
async function renderItemCanvas(it) {
  ...
  try {
    ...
    return await withTimeout(
      html2canvas(cardEl, { scale: 2, backgroundColor: "#ffffff", useCORS: true }),
      15000,
      "Rendering listing"
    );
  } finally {
